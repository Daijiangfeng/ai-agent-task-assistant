...
- 用途：企业级 AI Agent 任务执行助手（任务规划、多 Agent 协作、工具调用与高风险动作审批、任务暂停/恢复/取消/重试、任务模板、长期记忆、自我反思）。
...
| Frontend | `frontend/` | React SPA（四大界面：概览/任务控制台/任务模板/执行监控），经 Vite 代理 `/api` 调后端 |

## 关键能力速览（实现现状）

- **生产安全模式（P0-1）**：`ENVIRONMENT=development|production` 分区。production 下禁止一切静默降级
  （内存任务存储 / MemorySaver / 内存队列 / 内存短期记忆 / Mock 工具 email.send），启动即执行基础设施
  健康检查（PostgreSQL/Redis/向量库/LLM/存储，fail-fast）；`GET /health/live`（存活）与
  `GET /health/ready`（就绪，未就绪 503）供编排系统探针；任务接收端点（创建/执行/恢复/重试/模板运行/
  审批决策）生产模式下未就绪时 503 拒绝（`app/services/health.py` + `app/api/deps.py:require_ready`）。
  开发模式行为完全不变（保留降级）。
- **数据库迁移**：Alembic（`alembic.ini` + `migrations/`）。业务表结构以
  `app/models/task_record.py` 的 ORM 为单一来源；开发可用 create_all 引导，生产走
  `alembic upgrade head`；双路径结构一致性由 `tests/test_migrations.py` 保证；
...
- **搜索时效性**：`web_search` 集成意图分类（GENERAL_KNOWLEDGE / TIME_SENSITIVE / NEWS 等）、来源新鲜度评分与缓存绕过。
